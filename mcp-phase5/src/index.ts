...
async function apiFetch<T>(url: string, options: RequestInit = {}): Promise<{ data: T | null; error: string | null }> {
  try {
    const response = await fetch(url, {
      ...options,
      headers: { "User-Agent": "mcp-phase5-server", ...options.headers },
      signal: AbortSignal.timeout(10000),
    });
    if (!response.ok) {
      if (response.status === 401) return { data: null, error: "Unauthorized." };
      if (response.status === 403) return { data: null, error: "Forbidden — rate limit hit." };
...
interface GeoResult { results?: Array<{ latitude: number; longitude: number; name: string; country: string; timezone: string }> }
...
// ── CREATE MCP SERVER (one per connection) ──
function createMcpServer(): McpServer {
  const server = new McpServer({ name: "phase5-sse-server", version: "1.0.0" });

  server.tool("get_current_weather", "Gets current weather for any city.",
    ...
    async ({ city, units = "celsius" }) => {
      ...
      const weather = await apiFetch<WeatherResult>(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code,apparent_temperature,precipitation&temperature_unit=${units === "celsius" ? "celsius" : "fahrenheit"}&wind_speed_unit=kmh&timezone=${encodeURIComponent(timezone)}`);
  ...
  server.tool("get_weather_forecast", "Gets a multi-day weather forecast for any city.",
    ...
    async ({ city, days = 5, units = "celsius" }) => {
      ...
      const { latitude, longitude, name, country, timezone } = geo.data.results[0];
      const sym = units === "celsius" ? "°C" : "°F";
      const weather = await apiFetch<WeatherResult>(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_sum&temperature_unit=${units === "celsius" ? "celsius" : "fahrenheit"}&timezone=${encodeURIComponent(timezone)}&forecast_days=${days}`);
  ...
  server.tool("compare_weather", "Compares weather between two cities.",
    ...
    async ({ city1, city2, units = "celsius" }) => {
      ...
      const loc1 = geo1.data.results[0]; const loc2 = geo2.data.results[0];
      const sym = units === "celsius" ? "°C" : "°F";
  ...
  server.tool("get_github_user", "Gets public profile for any GitHub user.",
    { username: z.string() },
    async ({ username }) => {
  ...
  server.tool("list_user_repos", "Lists public repos for a GitHub user.",
    ...
    async ({ username, limit = 10 }) => {
      ...
      if (!repos.length) return { content: [{ type: "text", text: `No repos for ${username}.` }] };
      const list = repos.map((r, i) => `${i + 1}. ${r.name} ⭐${r.stargazers_count}\n   ${r.description || "No description"}\n   ${r.html_url}`).join("\n\n");
      return { content: [{ type: "text", text: `📦 ${repos.length} repos for ${username}:\n\n${list}` }] };
    }
  );

  server.tool("get_repo_info", "Gets details about a GitHub repository.",
  ...
  server.tool("search_github_repos", "Searches GitHub for repositories.",
  ...
  server.tool("get_readme", "Fetches the README of a GitHub repository.",
    { owner: z.string(), repo: z.string() },
    async ({ owner, repo }) => {
...
// ── EXPRESS APP ──
const app = express();
app.use(cors({ origin: "*", methods: ["GET", "POST", "DELETE"], allowedHeaders: ["Content-Type", "Authorization", "mcp-session-id"] }));
app.use(express.json());

// Sessions for both transport types
const streamableSessions = new Map<string, StreamableHTTPServerTransport>();
const sseSessions = new Map<string, SSEServerTransport>();

// 🏥 Health check
app.get("/health", (_req, res) => {
  res.json({ status: "ok", server: "phase5-mcp-sse", version: "1.0.0", activeSessions: streamableSessions.size + sseSessions.size, timestamp: new Date().toISOString() });
});

// ── STREAMABLE HTTP (modern protocol, what Claude Desktop uses) ──
// Handles POST / for new sessions and existing sessions
app.post("/", async (req: Request, res: Response) => {
  ...
  if (!isInitializeRequest(req.body)) {
    res.status(400).json({ error: "Must start with initialize request" });
    return;
  }
  ...
  const transport = new StreamableHTTPServerTransport({
    ...
    onsessioninitialized: (newSessionId) => {
      ...
      console.error(`Streamable session started: ${newSessionId}`);
    },
  });
  ...
  transport.onclose = () => {
    if (transport.sessionId) {
      ...
      console.error(`Streamable session ended: ${transport.sessionId}`);
    }
  };
...
// GET / for SSE stream in Streamable HTTP
...
// ── LEGACY SSE (kept for backwards compatibility) ──
app.get("/sse", async (req: Request, res: Response) => {
  ...
  req.on("close", () => {
    ...
    console.error(`SSE session ended: ${transport.sessionId}`);
  });
...
// 🚀 Start
app.listen(PORT, () => {
  console.error(`✅ Phase 5 MCP Server running on port ${PORT}`);
  console.error(`   🏥 Health:          http://localhost:${PORT}/health`);
  console.error(`   🆕 Streamable HTTP: http://localhost:${PORT}/`);
