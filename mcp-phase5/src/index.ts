...
// // ── KEY FIX: Create a NEW McpServer per connection ──
// // The SDK does not support reusing one server across multiple connections.
// // Each client that connects gets its own fresh McpServer instance.
...
//   res.json({ status: "ok", server: "phase5-mcp-sse", version: "1.0.0", activeSessions: sessions.size, timestamp: new Date().toISOString() });
// });
...
//   if (!transport) { res.status(404).json({ error: `Session ${sessionId} not found` }); return; }
...
app.get("/sse", async (req, res) => {
  ...
  // Keepalive ping every 30s to prevent Railway's load balancer from closing idle SSE connections
